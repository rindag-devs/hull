...
/* global default_theme, default_dark_theme, default_light_theme */
...
(function codeSnippets() {
  ...
  function update_play_button(pre_block, playground_crates) {
    ...
    const all_available = snippet_crates.every((elem) => playground_crates.indexOf(elem) > -1);
  ...
  Array.from(code_nodes).forEach((block) => {
...
aria-label="Show hidden lines"></button>';
    ...
    buttons.firstChild.addEventListener("click", (e) => {
      ...
      if (button.title === "Show hidden lines") {
        button.innerHTML = document.getElementById("fa-eye-slash").innerHTML;
        button.title = "Hide lines";
        button.setAttribute("aria-label", e.target.title);

        block.classList.remove("hide-boring");
      } else if (button.title === "Hide lines") {
        button.innerHTML = document.getElementById("fa-eye").innerHTML;
        button.title = "Show hidden lines";
        button.setAttribute("aria-label", e.target.title);
...
(function chapterNavigation() {
  document.addEventListener("keydown", (e) => {
    if (e.altKey || e.ctrlKey || e.metaKey || mdbook_something_else_has_focus(e)) {
      return;
    }
  ...
  Array.from(clipButtons).forEach((clipButton) => {
    clipButton.addEventListener("click", async (e) => {
      ...
      try {
        ...
        showTooltip(trigger, "Clipboard error!");
      }
    });
