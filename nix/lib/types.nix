...
let
  ...
  testCaseResultStatusStr = lib.types.strMatching "internal_error|accepted|wrong_answer|partially_correct|runtime_error|time_limit_exceeded|memory_limit_exceeded|file_error";

  testCaseResult = lib.types.submodule {
    options = {
      status = lib.mkOption {
        type = testCaseResultStatusStr;
        description = "The judgement status: `accepted`, `wrong_answer`, `partially_correct`, `runtime_error`, `time_limit_exceeded`, `memory_limit_exceeded`, `file_error`, or `internal_error`.";
      };
      score = lib.mkOption {
  ...
  checkReport = lib.types.submodule {
    options = {
      status = lib.mkOption {
        type = lib.types.strMatching "internal_error|accepted|wrong_answer|partially_correct|file_error";
      ...
      { name, config, ... }:
      {
        options = {
          ...
          memoryLimit = lib.mkOption {
            ...
            description = "WASM linear-memory and execution-stack byte ceiling for this specific test case.";
          };
          groups = lib.mkOption {
            type = listOf nameStr;
            default = [ ];
            ...
            type = submodule {
              options = {
                input = lib.mkOption {
                  ...
                  default = coerceStorePath problem.runtimeData.test_cases.${config.name}.data.input;
                  defaultText = "Loaded from runtime analysis data.";
                };
                outputs = lib.mkOption {
                  type = pathInStore;
                  ...
                  default = coerceStorePath problem.runtimeData.test_cases.${config.name}.data.outputs;
                  defaultText = "Loaded from runtime analysis data.";
                };
              };
            };
          ...
          inputValidation = lib.mkOption {
            ...
            default = {
              inherit (problem.runtimeData.test_cases.${config.name}.input_validation) status message traits;
              readerTraceStacks =
                problem.runtimeData.test_cases.${config.name}.input_validation.reader_trace_stacks;
              readerTraceTree = problem.runtimeData.test_cases.${config.name}.input_validation.reader_trace_tree;
            };
            defaultText = "Loaded from runtime analysis data.";
          };
        };
        ...
        options = {
          ...
          subtaskPredictions = lib.mkOption {
            ...
            description = ''
              ...
              and statuses, and then returns true if the prediction is met. Statuses use `accepted`, `wrong_answer`,
              `partially_correct`, `runtime_error`, `time_limit_exceeded`, `memory_limit_exceeded`,
              `file_error`, or `internal_error`.'';
          ...
          testCaseResults = lib.mkOption {
            ...
            ) problem.runtimeData.solutions.${config.name}.test_case_results;
            defaultText = "Loaded from runtime analysis data.";
          };
            ...
            default =
              map
                ...
                (
                  ...
                  }) problem.runtimeData.solutions.${config.name}.subtask_results
                );
            defaultText = "Loaded from runtime analysis data.";
          };
          score = lib.mkOption {
    ...
    submodule (args: {
      options = (programOptions problem args) // {
        ...
        testResults = lib.mkOption {
          ...
          readOnly = true;
          default = lib.mapAttrs (_: result: {
            inherit (result) status score message;
            readerTraceStacks = result.reader_trace_stacks;
            evaluatorTraceStacks = result.evaluator_trace_stacks;
          }) problem.runtimeData.checker.test_results;
          defaultText = "Loaded from runtime analysis data.";
        };
      };
    });
    ...
    submodule (args: {
      options = (programOptions problem args) // {
        ...
        testResults = lib.mkOption {
          ...
          readOnly = true;
          default = lib.mapAttrs (_: result: {
            inherit (result) status message traits;
            readerTraceStacks = result.reader_trace_stacks;
            readerTraceTree = result.reader_trace_tree;
          }) problem.runtimeData.validator.test_results;
          defaultText = "Loaded from runtime analysis data.";
        };
      };
    });
