      ...
      __functor =
        ...
        let
          ...
          modeConfig =
            {
              batch = {
                taskType = if graderSrc != null then taskTypeEnum.graderInteraction else taskTypeEnum.batch;
              };
              answerOnly = {
                # Lemon's answer-only mode does not match Hull's packaged answer files.
                taskType = taskTypeEnum.batch;
              };
            }
            .${mode};

          taskType = modeConfig.taskType;
...
in
{
  batch = args: mkLemonTarget (args // { mode = "batch"; });
  answerOnly = args: mkLemonTarget (args // { mode = "answerOnly"; });
}
