...

    {
      ...
      __functor =
          ...
          modeConfig =
            {
              ...
              stdioInteraction = {
                ...
                taskYaml = { };
                checkerFileName = "manager";
                graderFilePrefix = "stub";
              };
              answerOnly = {
                checkerPatch = {
                  checker = "::cplib_initializers::cms::checker::Initializer()";
        ...
        pkgs.runCommandLocal "hull-problemTargetOutput-${problem.name}-cms" { } ''
          ...
          cp ${compiledChecker}/bin/program $out/check/${modeConfig.checkerFileName}
          ${lib.concatMapAttrsStringSep "\n" (
            lang: src: "cp ${src} $out/sol/${modeConfig.graderFilePrefix}.${lang}"
          ) graderSrcs}
...
in
{
  batch = args: mkCmsTarget (args // { mode = "batch"; });
  stdioInteraction = args: mkCmsTarget (args // { mode = "stdioInteraction"; });
  answerOnly = args: mkCmsTarget (args // { mode = "answerOnly"; });
}
