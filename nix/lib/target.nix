...

{
  default = {
    ...
    __functor =
      ...
      in
      pkgs.runCommandLocal "hull-targetOutput-${name}-default" { } ''
        ${dataCommand}
  ...
  hydro =
    {
      # The problem type in Hydro's config.yaml.
      # Since Hull's judger is customizable, you need to manually map it.
      # Valid options include "default", "submit_answer", "interactive".
      type ? "default",
      ...
      graderCompileCommand ? "g++ -x c++ grader.code -c -o grader.o -std=c++23 -O3",

      # The name of the test case output that will be used as the output of the Hydro test case.
      outputName ? if type == "interactive" then null else "output",
        ...
        let
          ...
          configYamlContent = {
            ...
            subtasks = lib.imap0 (index: st: {
              ...
              type = "min"; # Hull's default subtask scoring is 'min'
              cases = map (tc: {
                input = "${tc.name}.in";
          ...
          participantProgramsCommand =
            let
              ...
              mkSolutionCopyCommand =
                solName: sol:
                lib.optionalString sol.participantVisibility "cp ${sol.src} $tmpdir/additional_file/solution_${solName}.code";
            in
